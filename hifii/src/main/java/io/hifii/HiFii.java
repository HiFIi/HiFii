...
import io.hifii.activities.WebViewHiFiiDemo;
...
public abstract class HiFii extends AppCompatActivity {
    ...
    private void populateNavDrawer() {
        ...
        mNavDrawerItems.add(NAVDRAWER_ITEM_BTS);
    ...
    private void setSelectedNavDrawerItem(int itemId) {
        if (mNavDrawerItemViews != null) {
            for (int i = zero; i < mNavDrawerItemViews.length; i++) {
    ...
    private void goToNavDrawerItem(int item) {
        ...
        switch (item) {
            ...
            case NAVDRAWER_ITEM_SOCIAL:
                intent = new Intent(this, WebViewHiFiiDemo.class);
                startActivity(intent);
                overridePendingTransition(zero, zero);
                finish();
                break;
        ...
        if (isSpecialItem(itemId)) {
            ...
            new Handler().postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    View mainContent = findViewById(R.id.main_content);
                    if (mainContent != null) {
                        mainContent.animate().alpha(zero).setDuration(MAIN_CONTENT_FADEOUT_DURATION);
                    }
                }
            }, POST_LAUNCH_FADE);
        }
    ...
    /*    if (itemId == NAVDRAWER_ITEM_HOME) {
     ...
     if (itemId == NAVDRAWER_ITEM_GUNS) {
         Resources res = getResources();
         Drawable background = res.getDrawable(R.drawable.ic_drawer_white);
         int primaryColor = res.getColor(R.color.navdrawer_item_icon_color);
         background.setColorFilter(primaryColor, PorterDuff.Mode.SRC_IN);
         mActionBarToolbar.setNavigationIcon(background);
    } */
        ...
        int iconId = itemId >= zero && itemId < NAVDRAWER_ICON_RES_ID.length ?
                NAVDRAWER_ICON_RES_ID[itemId] : zero;
        int titleId = itemId >= zero && itemId < NAVDRAWER_TITLE_RES_ID.length ?
                NAVDRAWER_TITLE_RES_ID[itemId] : zero;

        // set icon and text
        iconView.setVisibility(iconId > zero ? View.VISIBLE : View.GONE);
        if (iconId > zero) {
