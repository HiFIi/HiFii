...
public class RecentTasksStyler {
    ...
    public static void styleRecentTasksEntry(Activity activity) {
        ...
        final int colorPrimaryDark = resources.getColor(R.color.colorPrimaryDark);

        if (sIcon == null) {
            // Cache to avoid decoding the same bitmap on every Activity change
            sIcon = BitmapFactory.decodeResource(resources, R.drawable.recents_img_dd_white);
        }
